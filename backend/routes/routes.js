...
router.get('/tours/availability', async (_req, res) => {
  const sql = `with booked as (
      select
        ts.id as schedule_id, ts.tour_id, ts.schedule_time, ts.seats_available,
        coalesce(sum(r.seats_reserved), 0) as seats_reserved
      from tourapp.tour_schedules ts
      left join tourapp.reservations r on r.tour_schedule_id = ts.id
      group by ts.id, ts.tour_id, ts.schedule_time, ts.seats_available)
    SELECT
      b.schedule_id, b.tour_id, t.name as tour_name,
      b.schedule_time, b.seats_available,
      (b.seats_available - b.seats_reserved) as seats_remaining
    from booked b
    join tourapp.tours t on t.id = b.tour_id
    where b.schedule_time > NOW()
      and (b.seats_available - b.seats_reserved) > 0
